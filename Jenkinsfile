...
// which are part of the KUmsl repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="kumsl-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /kumsl-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "devronkim-kumsl/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from devronkim-kumsl/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'devronkim-kumsl/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from devronkim-kumsl/develop"
            }
          }

          if (hasCache) {
            sh "cd /kumsl-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /kumsl-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /kumsl-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /kumsl-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /kumsl-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /kumsl-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /kumsl-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
