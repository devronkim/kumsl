...
const char * const BITCOIN_CONF_FILENAME = "kumsl.conf";
const char * const BITCOIN_PID_FILENAME = "kumsld.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "kumsl" is a composite category enabling all KUmsl-related debug output
                if(ptrCategory->count(std::string("kumsl"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KUmslCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KUmslCore
    // Mac: ~/Library/Application Support/KUmslCore
    // Unix: ~/.kumslcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KUmslCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KUmslCore";
#else
    // Unix
    return pathRet / ".kumslcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty kumsl.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override kumsl.conf
