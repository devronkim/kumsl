...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 KUMSL irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on kumslcentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in kumsl.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!fMnFound) {
        ...
        resultsObj.push_back(Pair("kumsl.conf", statusObj));
...
[[ noreturn ]] void gobject_help()
{
    throw std::runtime_error(
            ...
            "  vote-conf          - Vote on a governance object by masternode configured in kumsl.conf\n"
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* KUmsl features */
    { "kumsl",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "kumsl",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "kumsl",               "gobject",                &gobject,                true,  {} },
    { "kumsl",               "voteraw",                &voteraw,                true,  {} },
