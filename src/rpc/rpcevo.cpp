...
// Allows to specify KUmsl address or priv key. In case of KUmsl address, the priv key is taken from the wallet
...
void protx_register_fund_help()
{
    throw std::runtime_error(
            ...
            "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 1000 KUmsl\n"
            "to the address specified by collateralAddress and will then function as the collateral of your\n"
            "masternode.\n"
            "A few of the limitations you see in the arguments are temporary and might be lifted after DIP3\n"
            "is fully deployed.\n"
            "\nArguments:\n"
            "1. \"collateralAddress\"   (string, required) The kumsl address to send the collateral to.\n"
            ...
            "7. \"payoutAddress\"       (string, required) The kumsl address to use for masternode reward payments.\n"
...
UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > paramIdx + 6) {
        ...
        if (!fundAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid KUmsl address: ") + request.params[paramIdx + 6].get_str());
    }
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() >= 6) {
        ...
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid KUmsl address: ") + request.params[5].get_str());
...
void protx_update_registrar_help()
{
    throw std::runtime_error(
            ...
            "4. \"payoutAddress\"       (string, required) The kumsl address to use for masternode reward payments\n"
...
UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 4) {
        ...
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid KUmsl address: ") + request.params[4].get_str());
        FundSpecialTx(tx, ptx, feeSourceAddress.Get());
