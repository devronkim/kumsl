...
*
*   Why is this needed?
*   - In KUmsl some blocks are superblocks, which output much higher amounts of coins
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew) const
{
    ...
    LogPrintf("CMasternodeBlockPayees::%s -- ERROR: Missing required payment, possible payees: '%s', amount: %f KUMSL\n", __func__, strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
