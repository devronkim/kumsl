...
# kumsl_hash
RUN git clone https://github.com/devronkim/kumsl_hash
RUN cd kumsl_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} kumsl
RUN useradd -u ${USER_ID} -g kumsl -s /bin/bash -m -d /kumsl kumsl
...
RUN mkdir /kumsl-src && \
  ...
  chown $USER_ID:$GROUP_ID /kumsl-src && \
